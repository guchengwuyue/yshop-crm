...
public class CrmCustomerController {
    ...
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('crm:open-customer:receive')")
    ...
    @Operation(summary = "规则配置")
    public CommonResult<Boolean> saveRedisSet(@Valid @RequestBody ConfigSetVO configSetVO) {
        RedisUtil.set("customerNum",configSetVO.getCustomerNum());
        RedisUtil.set("notRecordDay",configSetVO.getNotRecordDay());
        RedisUtil.set("notSuccessDay",configSetVO.getNotSuccessDay());
        return success(true);
    }

    @GetMapping("/getRedisSet")
    @Operation(summary = "获得规则配置")
    public CommonResult<ConfigSetVO> getRedisSet() {
        ConfigSetVO configSetVO = ConfigSetVO.builder()
                .customerNum(RedisUtil.get("customerNum"))
                .notRecordDay(RedisUtil.get("notRecordDay"))
                .notSuccessDay(RedisUtil.get("notSuccessDay"))
                .build();
