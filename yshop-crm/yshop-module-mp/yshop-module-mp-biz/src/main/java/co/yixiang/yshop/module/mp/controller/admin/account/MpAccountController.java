...
public class MpAccountController {
    ...
    public CommonResult<MpAccountRespVO> getAccount(@RequestParam(value = "id",required = false) Long id) {
    ...
    @PutMapping("/set-main")
    @Operation(summary = "设置公众号未主账户")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('mp:account:set-main')")
    public CommonResult<Boolean> setAccountMain(@RequestParam("id") Long id) {
        mpAccountService.setMain(id, MpAccountEnum.IS_MINI_0.getValue());
        return success(true);
    }
