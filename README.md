...
物质仓库库存管理系统，面向中小企业仓管、采购与业务协同，覆盖多规格、出入库盘库、扫码检索、库存时序与单据条码打印。

把库存账做准、单据流程做规范、物资流转看得清。

**定位：** 建档 → 入库 → 出库 → 盘库 → 查询 / 时序 → 条码打印存档
...
| 项目| 项目管理、项目任务、项目文档、项目动态、项目任务、任务日志等|
...
| 模块 | 功能 |
|------|------|
| 商品管理 | 多规格 SKU（颜色、尺寸等），按规格维护条形码与库存维度 |
| 商品入库 | 关联供应商创建入库单，支持扫码枪条码搜索，审核后增加库存 |
| 商品出库 | 关联客户 / 领用场景创建出库单，支持扫码枪条码搜索，审核后扣减库存 |
| 商品盘库 | 按账面发起盘点，扫码录入实盘，自动生成盈亏并调整库存 |
| 库存查询 | 按商品、规格查看当前库存余额 |
| 库存时序 | 增减变化时间线，还原「何时、因何单、变多少」 |
| 单据打印 | 入/出/盘库单 A5 打印，单据支持商品条形码展示 |
| 扫码检索 | 入库、出库、盘库开单支持扫码枪条形码搜索选品 |
| 审核机制 | 入/出/盘库单审核生效 / 反审核回退 |
| 客户管理 | 客户档案，出库可关联去向 |
| 供应商管理 | 供应商档案，入库可关联来源 |
| 移动端uniapp | 支持小程序与h5:手机端入库、出库、盘库、库存查询等 |


**业务闭环：** 建商品/规格/条码 → 建客户/供应商 → 扫码入库 → 扫码出库 → 扫码盘库 → 库存查询与时序 → 条码打印存档
