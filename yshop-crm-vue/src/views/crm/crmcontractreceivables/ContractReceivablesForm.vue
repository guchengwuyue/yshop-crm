<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    <el-alert title="审核不通过，不可修改数据，请重新添加合同" type="error" v-else-if="formData.checkStatus == 3" style="margin-bottom:20px" />
    <el-form
      ref="formRef"
      :model="formData"
      :rules="formRules"
    ...
    >
      ...
      <el-form-item label="收款方式" prop="payType">
        <el-radio-group v-model="formData.payType">
          <el-radio :value="1">默认方式</el-radio>
          <el-radio :value="2">在线收款</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="回款金额" prop="money">
        <el-input v-model="formData.money" placeholder="请输入回款金额" />
      </el-form-item>
      <el-form-item label="回款账户" prop="accountId" v-show="formData.payType == 1">
      ...
      <el-form-item label="回款日期" prop="returnTime" v-show="formData.payType == 1">
        <el-date-picker
      ...
      <el-form-item label="审批信息" prop="flowAdminId">
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    if(formData.value.checkStatus == 2 || formData.value.checkStatus == 1 || formData.value.checkStatus == 3 ){
    ...
    formData.value.flowAdminId = flowAdminsId
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    //支付
    if(formData.value.payType == 2){
      ContractReceivablesApi.pay({from:'',paytype:'weixin',uni:formData.value.number})
    }
    // 发送操作成功的事件
    emit('success')
  } finally {
