<template>
      ...
      <el-table-column label="单价" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.price`" :rules="formRules.price" class="mb-0px!">
            <el-input type="number" @input="changeUnitPrice(row)" v-model="row.price" placeholder="请输入单价" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column label="数量" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.nums`" :rules="formRules.nums" class="mb-0px!">
            <el-input type="number" v-model="row.nums" @input="changeNumber(row)" placeholder="请输入数量" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column label="小计" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.subtotal`" :rules="formRules.subtotal" class="mb-0px!">
            <el-input v-model="row.subtotal" readonly placeholder="请输入小计" />
          </el-form-item>
        </template>
      </el-table-column>
  ...
  <el-row justify="center" class="mt-3">
    <el-button @click="handleAdd" round>+ 添加商机产品关联</el-button>
  </el-row>
...
const props = defineProps<{
  businessId: undefined // 商机id（主表的关联字段）
}>()
...
const handleAdd = () => {
  ...
  row.businessId = props.businessId
